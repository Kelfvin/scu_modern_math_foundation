...
（2）代入 $x^{(0)}$ 计算得 $x^{(1)} = [x_1^{(1)}, x_2^{(1)}, x_3^{(1)}]^T$

\subsubsection*{Gauss-Seidel}

（1）参照Jacobi迭代，将$Ax=b$改写。只不过在计算$x_i^{(k+1)}$时，使用已经计算出的$x_j^{(k+1)}$，而不是$x_j^{(k)}$。口诀：“\textbf{右边的比左边的x的下标大就是k，比左边的x的下标小就是k+1}”
...
写出Recharson迭代格式，并给出收敛条件和最优参数。

（1）令 $B=I-\alpha A$

（2）迭代格式：$x^{(k+1)} = Bx^{(k)} + \alpha b$ 

（3）收敛条件：

\textcircled{1} 计算 $\lambda_A$: $|A-\lambda I|=0$

技巧：3阶的行列式化解后为 $p(\lambda) = \lambda^3 + a\lambda^2 + b\lambda + c$。一般有整根 $r$，$r$ 一定为 $c$ 的因子。取 $c$ 的因子 $\pm 1 \pm 2 \cdots$ 进去一个个试，找到整根后，使用多项式除法将 $p(\lambda)$ 除以 $(\lambda - r)$，得到一个二次多项式，再用求根公式求出另外两个根。

\textcircled{2} $\lambda_B = 1-\alpha \lambda_A$，$\rho(B)<1 \Rightarrow |1-\alpha \lambda_A|<1$
...
\[
|\lambda_i -a_{ii}| \leq \sum_{j=1,j\neq i}^n |a_{ij}| \quad \text{依次对} i \text{行除了} a_{ii} \text{外的其他元素求和}
\]

得到$\lambda_A$区间范围 $\lambda_{1} \in [a,b]$，$\lambda_{2} \in [b,c]$，$\lambda_{3} \in [d,e]$

(2) 位移，将原本第二模大特征值变为最大或最小特征值

\textcircled{1}确定位移量

对区间整体进行位移，将第二大特征值区间移动到模最大或最小，可在数轴上标出辅助思考：

$\lambda_1+p \in [a+p,b+p] \quad \lambda_2+p \in [b+p,c+p] \quad \lambda_3+p \in [pd+p,e+p]$

或 $\lambda_1 -p \in [a-p,b-p] \quad \lambda_2 -p \in [b-p,c-p] \quad \lambda_3 -p \in [d-p,e-p]$

\textcircled{2} 根据位移量$p$构造新矩阵$B$：

\begin{itemize}
    \item $B=A-pI$，$B$的特征值为$\lambda_A -p$
    \item $B=A+pI$，$B$的特征值为$\lambda_A +p$
\end{itemize}
...
\begin{itemize}
    \item A第2大模特征值 $\rightarrow$ B最大特征值 $\Rightarrow$ 幂法解B
    \item A第2大模特征值 $\rightarrow$ B最小特征值 $\Rightarrow$ 反幂法解B
\end{itemize}
...
\left\{
    \begin{aligned}
        ...
        \lim_{k \to \infty} \max(x^{(k)}) &= \lambda_{B1} \; \text{最大特征值}
    \end{aligned}
\right.
\]
...
\left\{
    \begin{aligned}
        ...
        \lim_{k \to \infty} \max(x^{(k)}) &= \lambda_{Bn} \; \text{最小特征值}
    \end{aligned}
\right.
\]
