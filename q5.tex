...
\textbf{局部收敛判定}: 如果 $|\varphi'(\alpha)|<1$，且在 $\alpha$ 邻近连续，则初值够近就收敛。

\textbf{收敛阶数}：设 $x=\varphi(x)$的根具有连续$p$阶导数，且在根处 $\varphi'(\alpha)=\varphi''(\alpha)=...=\varphi^{(p-1)}(\alpha)=0$，$\varphi^{(p)}(\alpha) \neq 0$，则迭代格式的收敛阶数为 $p$ 。
