...
给定m+1个点x和y，用最小二乘法确定最优的拟合曲线；注意下标从0开始。

设 $y=a+bx$，然后代入$x_i$和$y_i$，得到m+1个方程（超定方程组）：
...
对于n次多项式拟合，只需将矩阵$\mathbf{A}$的列数改为$n+1$，每一列对应$x^0,x^1,\dots,x^{n}$即可。$n$次的法方程组为：
...
设置多项式为 $f(x) = a_0 \varphi_0(x) + a_1 \varphi_1(x)$。$\varphi_0(x)=1$，$\varphi_1(x)=x$。
